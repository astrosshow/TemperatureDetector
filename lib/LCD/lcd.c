...
void lcd_fb_enable(void)
{
    ...
    ECLIC_SetCfgNlbits(0);                      // No priority bits split
    ECLIC_SetLevelIRQ(USART0_IRQn, 0);     // Level = 0
    ECLIC_SetPriorityIRQ(USART0_IRQn, 0);  // Priority = 0    
    ECLIC_EnableIRQ(USART0_IRQn);               // Enable USART0 interrupt
